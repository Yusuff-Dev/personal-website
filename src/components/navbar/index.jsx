    ...
    <>
      <nav className='fixed top-0 right-0 z-50 py-10'>
        <div className="container flex items-center justify-end">
          <div onClick={handleOpen} className="rounded-full bg-gray-700 px-3 flex items-center h-12 relative select-none cursor-pointer">
            ...
            <button className={`absolute top-1 rounded-full w-10 h-10 flex flex-col gap-1 items-center justify-center bg-[#CCCCCC] ${isActive ? 'left-1' : 'left-full -translate-x-[calc(100%+4px)]'} duration-500 ease-in`}>
              <span className={`border border-gray-700 rounded-full block w-4 bg-gray-700 ${isActive ? 'rotate-45 translate-y-[3px]' : ''} duration-300`}></span>
              <span className={`border border-gray-700 rounded-full block w-4 bg-gray-700 ${isActive ? '-rotate-45 -translate-y-[2.4px]' : ''} duration-300`}></span>
            </button>
          </div>
        </div>
      </nav>
